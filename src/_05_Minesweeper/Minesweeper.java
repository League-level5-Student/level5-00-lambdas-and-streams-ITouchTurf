...
public class Minesweeper extends PApplet {
	 ...
	 */
	boolean checkWin() {
		return cells.stream().filter(s -> s.revealed).filter(s -> !s.mine).count() == 0;
	...
	void revealCell(Cell cell) {
		if(!cell.mine && cell.revealed == false) {
			...
			if(cell.minesAround == 0) {
				getNeighbors(cell).stream()./*filter(c -> c.minesAround == 0).*/forEach(c -> revealCell(c));
	...
	void setNumberOfSurroundingMines() {
		cells.stream().forEach(
				c -> c.minesAround = getNeighbors(c).stream().mapToInt(
						cell -> cell.mine? 1:0).sum()  );
	}
